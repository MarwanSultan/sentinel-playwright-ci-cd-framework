...
test.describe("Smoke Tests - Core Functionality Verification", () => {
  ...
  test.describe("Critical Features Detection", () => {
    test("SMOKE-06: Benefits section accessible", async ({ page }) => {
      await page.goto(BASE_URL);
      const benefitsLink = page.locator("a").filter({ hasText: /benefits/i });
      expect(await benefitsLink.count()).toBeGreaterThan(0);
    });
  ...
  test.describe("Page Performance", () => {
    test("SMOKE-11: Homepage loads within reasonable time", async ({
      page,
    }) => {
    ...
    test("SMOKE-12: No excessive JavaScript errors on page", async ({
      page,
    }) => {
      let errorCount = 0;
