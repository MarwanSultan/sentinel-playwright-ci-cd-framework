...
| MSG-17 | Send message with emoji | 1. Include emoji in message<br>2. Send and receive | Emoji rendered correctly on both ends |
...
const metrics = await page.evaluate(() => {
  return {
    ...
    ttfb: performance.timeOrigin + performance.timing.responseStart - 
...
page.on('response', response => {
  const duration = response.url().includes('api/')
...
for (const benefit of benefitTypes) {
  test(`Submit ${benefit.type} application`, async ({ page }) => {
    ...
    const confirmation = await page.locator(
      `text=${benefit.confirmationMessage}`
    );
...
clinics.forEach(clinic => {
  availableDates.forEach(date => {
    test(`Schedule appointment at ${clinic.name} on ${date}`, 
      async ({ page }) => {
...
const testData = csv.parse(
  fs.readFileSync('test-data/claims.csv'), 
  { columns: true }
);

testData.forEach(row => {
  test(`View ${row.claim_type} claim with ${row.status} status`, 
    async ({ page }) => {
